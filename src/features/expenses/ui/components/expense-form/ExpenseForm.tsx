...
export const ExpenseForm = (props:Props) => {
    ...
    useEffect(() => {
        setParticipants(members.map(member => ({user: member, amount: 0, isParticipant: true})))
    }, [members])

    const orderParticipants = (participantA:Participant, participantB:Participant) => {
        if(participantA.user.name > participantB.user.name) return 1
        else return -1
    }

    const setParticipant= (participant: Participant) => {
        setParticipants(prev => [...prev.filter(({user}) => user.id !== participant.user.id), participant])
    }
    const getPayers = (participants: Participant[]): Payer[] => {
        return participants
        .filter(participant => participant.amount > 0 && participant.isParticipant)
        .map(({amount, user}) => ({id: user.id, name: user.name, amount}))
    }
    ...
    const handleErrors = (): string => {
        ...
        if(getPayers(participants).length === 0) errors+='Debe haberse realizado algún pago para crear un gasto\n'
        setError(errors)
        return errors
    }
            ...
            <ul className={styles.members_list}>
                ...
                .map((participant) => (
                    ...
                    >
                        ...
                        onChange={() => setParticipant({...participant, isParticipant: !participant.isParticipant})}
                        />
                        {participant.user.name}
                        <input
                        type="number"
                        value={participant.amount}
                        onChange={e => setParticipant({...participant, amount: parseFloat(e.target.value)})}
                        />
                        €
                    </li>
                ))}
