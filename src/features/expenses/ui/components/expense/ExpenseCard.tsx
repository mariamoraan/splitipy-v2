        ...
        <div className={styles.wrapper}>
            ...
            <ul className={styles.payers_list}>
                {expense.payers.map((payer) => (
                    <li key={`${payer.name}-${payer.amount}`}>
                        <strong>{payer.name}</strong> pagó{' '}
                        <strong>{payer.amount}</strong>
                    </li>
                ))}
            </ul>
            <p>Deudas...</p>
            <ul className={styles.debts_list}>
                {calculatedDebts.map((debt) =>
                    debt.amount > 0 ? (
                        <li key={debt.id}>
                            <strong>{debt.name}</strong> debe{' '}
                            <strong>{debt.amount}€</strong>
                        </li>
                    ) : (
                        <li key={debt.id}>
                            A <strong>{debt.name}</strong> le deben{' '}
                            <strong>{Math.abs(debt.amount)}€</strong>
                        </li>
                    )
                )}
            </ul>
