...
export class CalculateBalanceQuery implements Query<Balance[], Expense[]>  {
    ...
    async execute(expenses: Expense[]):Promise<Balance[]> {
        ...
        const payers = Object.values(summary).filter(({amount}) => amount < 0).map(({id, amount}) => ({id, amount}))
        const debtors = Object.values(summary).filter(({amount}) => amount > 0).map(({id, amount}) => ({id, amount}))
                ...
                for(const debtor of debtors) {
                    if(debtor.amount >= 0.01) {
                        ...
                        const partialDebt = 
                            ...
                            : Number(
                                ((Math.abs(payer.amount) / debtor.amount) * debtor.amount).toFixed(2)
                             )
                        
                        if(partialDebt > 0) {
                            modifyElem(
                                payers, 
                                payers.indexOf(payer), 
                                {...payer, amount: Math.abs(payer.amount) - partialDebt}
                            )
                            modifyElem(
                                debtors, 
                                debtors.indexOf(debtor), 
                                {...debtor, amount: partialDebt}
                            )
                            ...
                            if(!from || !to) return []
                            console.log(`from ${from.name} (${from.id}) to ${to.name} ${partialDebt}`)
