...
export const GroupForm = (props: Props) => {
    ...
    const [group, setGroup] = useState<Omit<Group, 'id'>>({...VOID_GROUP_WITHOUT_ID})
    ...
    const handleErrors = () => {
        ...
        if(group.members.length < 2) errors+='There must be at least 2 members in the group\n'
        setError(errors)
        return errors
    }
    ...
    const createValue = async(option: Option): Promise<Option | null> => {
        ...
        if(!user) {
            const newUser = await AuthLocator.getCreateUser().execute({name: option.value})
        ...
        <div className={styles.wrapper}>
            ...
            >
                <fieldset>
                    ...
                    placeholder="https://..."
                    value={group.image}
                    onChange={e => setGroup(prev => ({...prev, image: e.target.value}))}
                    />
                </fieldset>
                <MultipleSelector
                    id="group_members"
                    options={
                        ...
                        .map(({id, name}) => ({value: id, label: name, isFixed: false}))
                    }
                    createValue={createValue}
                    fixedItems={currentUser ? [{label: currentUser.name, value: currentUser.id, isFixed: true}] : []}
                    updateSelection={(items: Option[]) => setGroup(prev => ({...prev, members: items.map(({value}) => value)}))}
                />
