...
const SearchBar = () => {
    ...
    const isSelectedFromOptions = !!options.find(
...
const OptionsDisplayer = () => {
    const { options, search, setSearch, areOptionsVisible, items } = useContext(
        MultipleSelectorContext
    )
    ...
    const getCoincidences = () => {
        const coincidences = options.filter(
            (option) =>
                option.label
                    .toUpperCase()
                    .includes(search.label.toUpperCase()) ||
    ...
    const isOptionChoosen = (option: Option) =>
        !!items.find((item) => item.value === option.value)
    const getAvailableOptions = () =>
        options.filter((option) => !isOptionChoosen(option))
    ...
    if (!areOptionsVisible && (!search?.label || coincidences.length === 0))
        return null
